...
async def analyze(
    ...
    try:
        ...
        preview = results["data"].head(200).to_dict(
            orient="records"
        )
        ...
        response = {
            ...
            "grouped": results["grouped"].to_dict(
                orient="records"
            ),
            "insights": results["insights"],
            "charts": charts,
            "kpis": results.get("kpis", {}),
            "corr_matrix": results.get(
                "corr_matrix",
                {}
            ),
        }
...
async def insights(
    ...
    if k.get("missing_count", 0) > 0:
        ...
        insights_list.append(
            f"{k['missing_count']} missing values detected and cleaned."
        )

        recommendations.append(
            "Improve upstream data quality collection."
        )

    if k.get("duplicates", 0) > 0:

        insights_list.append(
            f"{k['duplicates']} duplicate rows removed."
        )
    ...
    if not insights_list:
        ...
        insights_list.append(
            "Dataset quality is excellent with minimal inconsistencies."
        )

        recommendations.append(
            "Maintain current data governance standards."
        )
